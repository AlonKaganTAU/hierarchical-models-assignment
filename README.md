...
- `analysis/02_fit_models.R` — fits Models 0-2 with `glmer()` (unconditional/ICC,
  ...
  (`anova()`), and `r2_pseudo()` for the random-effect-variance effect size. Writes
  `analysis/model_log.txt` and caches fitted models to `analysis/fits/` (not tracked
  in git — rerun the script to regenerate).
...
Requires R with `dplyr`, `tidyr`, `readr`, `purrr`, `stringr`, `ggplot2`, `lme4`,
