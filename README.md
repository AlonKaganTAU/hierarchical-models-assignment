...
(`stay ~ reward_oneback * K_c + (reward_oneback | subject)`, family =
...
- `data/trial_level.csv` — processed trial-level data (subject, block_number,
...
- `analysis/01_build_trial_level_data.R` — builds `data/trial_level.csv` from the raw
  ...
  the analysis sample (one row per subject, joined back onto the trial-level
...
`lmerTest`, `performance`, `parameters`, `marginaleffects`, `patchwork`.
